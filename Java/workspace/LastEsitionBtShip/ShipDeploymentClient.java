...
public class ShipDeploymentClient extends UnicastRemoteObject implements Play, MouseMotionListener, KeyListener{
	...
	private void initMyGrid() {
		...
		for(int j=1; j<=10; j++){
			for(int i=1; i<=10; i++){
				...
				myGameGrid[i][j].addMouseMotionListener(ShipDeploymentClient.this);
				myGameGrid[i][j].addKeyListener(ShipDeploymentClient.this);
	...
	private void initOtherGrid() {
		for(int j=1; j<=10; j++){
			for(int i=1; i<=10; i++){
				...
				otherGameGrid[i][j].setType(null);
			...
			btnStart.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					try {
						...
						Naming.rebind("//192.168.0.1/client", ShipDeploymentClient.this);
						
						//Naming.rebind("client", ShipDeploymentClient.this);
					} catch (Exception ex) {
						ex.printStackTrace();
					}
	 ...
	 * @throws IOException 
	 */
	public ShipDeploymentClient() throws IOException {
		super();
		initialize();
	}
	...
	private JPanel getJContentPane() {
		if (jContentPane == null) {
			...
			lblPeiceEnd.setBounds(new java.awt.Rectangle(805,17,196,21));
			lblPeiceEnd.setText("");
			...
			jContentPane.addKeyListener(ShipDeploymentClient.this);
		}
		return jContentPane;
	}
	...
	class OtherGridListener implements MouseListener {
		...
		public void mouseClicked(MouseEvent e) {
			...
			if (connected && turn == 2 && selected.getCurrState() == null) {
				try {
					...
					JOptionPane.showMessageDialog(frame, "Hardware Failed");
					System.exit(0);
				}
			}
		}
